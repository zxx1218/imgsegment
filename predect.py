...
# 处理预测结果并生成结果图像
def generate_result_images(pred, image):
    """
    处理预测结果并生成结果图像
    ...
    # 生成结果图像
    image_masked_resized = image_masked.resize(scaled_size)  # 带掩膜的图像
    image_resized_resized = image_resized.resize(scaled_size)  # 调整大小的原始图像
    pred_pil_resized = pred_pil.resize(scaled_size)  # 预测结果图像
    
    return image_masked_resized, image_resized_resized, pred_pil_resized


# 主函数(image_path为待分割图,model_path为选择模型的路径)
def main(image_path, model_path, large_or_tiny):
    """
    参数:
    - image_path: 分割图像路径
    - model_path: 使用pth模型存储路径
    - large_or_tiny: 使用哪一种模型参数配置
    """
    # 加载模型
    if large_or_tiny == "large":
        birefnet = load_model(model_path, "large")
    elif large_or_tiny == "tiny":
    ...
    # 生成结果图像
    image_masked_resized, image_resized_resized, pred_pil_resized = generate_result_images(pred, image)
    
    # 返回结果图像
    return image_masked_resized, image_resized_resized, pred_pil_resized

# if __name__ == '__main__':
#     image_masked, image_resized, pred_image = main()
